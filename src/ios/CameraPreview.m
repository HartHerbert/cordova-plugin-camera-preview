...
#define POOKYPET_PHOTO_PREFIX @"pookypet_photo_"
...
- (void) invokeTakePicture:(CGFloat) width withHeight:(CGFloat) height withQuality:(CGFloat) quality{
    ...
    [self.sessionManager.stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler:^(CMSampleBufferRef sampleBuffer, NSError *error) {
        ...
        if (error) {
            ...
            [self.commandDelegate sendPluginResult:result callbackId:self.onPictureTakenHandlerId];
            self.onPictureTakenHandlerId = nil;
...
- (NSString*)tempFilePath:(NSString*)extension
{
    ...
    int i = 1;
    do {
        filePath = [NSString stringWithFormat:@"%@/%@%03d.%@", docsPath, POOKYPET_PHOTO_PREFIX, i++, extension];
